...
{
#ifdef DEBUG
    NSLogD(@"%@ undefinedKey %@", self.class, key);
#endif
    return nil;
}
...
{
#ifdef DEBUG
    NSLogD(@"%@ undefinedKey %@ and value is %@", self.class, key, value);
#endif
}
...
- (id)objectForModelClass:(Class)modelClass fromDict:(NSDictionary *)dict withJSONKeyDict:(NSDictionary *)YYJSONKeyDict
{
    ...
    [YYJSONKeyDict enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        ...
        // 服务器有字符串为"id"的字段 这边改为Id。加点代码然后修正.
        // array 为字典里面的key值. 下面的 key 则为 对象的属性key
        if (![array containsObject:key])
        {
            for (NSString * string in array)
            {
                if ([string compare:(NSString *)key  options:NSCaseInsensitiveSearch] == NSOrderedSame)
                {
                    [muDict setObject:[dict objectForKey:string] forKey:key];
                }
            }
        }
